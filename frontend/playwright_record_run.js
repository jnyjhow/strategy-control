...
async function main() {
  ...
  try {
      ...
      if (await dialog.count() > 0) {
        ...
        if (await inputs.count() > 0) {
          ...
          try {
            ...
            const nameInput = dialog.locator('input[placeholder*="Nome"], input[placeholder*="Nome"], label:has-text("Nome") input').first()
            if (await nameInput.count() > 0) await nameInput.fill('E2E UI Test')
          } catch (e) {}
          try {
            const emailInput = dialog.locator('input[type="email"], input[placeholder*="E-mail"], label:has-text("E-mail") input').first()
            if (await emailInput.count() > 0) await emailInput.fill('e2e+test@example.com')
          } catch (e) {}
          try {
            const cpfInput = dialog.locator('input[placeholder*="CPF"], label:has-text("CPF") input, input[name="cpf"]').first()
            if (await cpfInput.count() > 0) await cpfInput.fill('00000000000')
          } catch (e) {}
    ...
    try {
      const clients = await page.evaluate(async (apiBase) => {
        try {
      ...
      if (clients && Array.isArray(clients)) {
        const found = clients.find(c => (c.cliente && c.cliente.email === 'e2e+test@example.com') || c.cliente?.name === 'E2E UI Test')
        if (found) {
    ...
    try {
      await page.waitForTimeout(1000)
      const created = await page.locator('text=E2E UI Test').first()
      if (await created.count() > 0) {
        ...
        const byEmail = await page.locator('text=e2e+test@example.com').first()
    ...
    try {
      // Find table rows and their action buttons; prefer row that contains our email
      const row = await page.locator('div.ClientsTable').locator('tr:has-text("e2e+test@example.com"), tr:has-text("E2E UI Test")').first()
      if (await row.count() > 0) {
        // open the actions menu inside this row
        const actionBtn = row.locator('button:has-text("\u22EE"), button:has-text("...")').first()
        if (await actionBtn.count() === 0) {
          // try the dots icon wrapper
          const anyAction = row.locator('button').filter({ hasText: '' }).first()
          if (await anyAction.count() > 0) await anyAction.click().catch(()=>{})
        } else {
          await actionBtn.click().catch(()=>{})
        }
        await page.waitForTimeout(400)
        // click Edit option in the opened menu
        const editInMenu = page.locator('button:has-text("Editar")').first()
        if (await editInMenu.count() > 0) {
          await editInMenu.click().catch(()=>{})
          console.log('clicked Editar in row menu')
          ...
          if (await nameInput2.count() > 0) {
            await nameInput2.fill('E2E Updated').catch(()=>{})
        ...
        console.log('could not find table row for created client to open actions menu')
      }
    ...
    try {
      const row2 = await page.locator('div.ClientsTable').locator('tr:has-text("E2E Updated"), tr:has-text("E2E UI Test")').first()
      if (await row2.count() > 0) {
        // open actions
        const actionBtn2 = row2.locator('button:has-text("\u22EE"), button').first()
        if (await actionBtn2.count() > 0) {
          await actionBtn2.click().catch(()=>{})
          await page.waitForTimeout(300)
          // click Remover in the menu
          const remover = page.locator('button:has-text("Remover"), button:has-text("Remover")').first()
          if (await remover.count() > 0) {
            ...
            console.log('Remover option not found in menu')
          }
        }
      } else {
    ...
    try {
      if (!createdClientId) {
        ...
        const clients2 = await page.evaluate(async (apiBase) => {
          try { const res = await fetch(apiBase + '/api/clients'); if (!res.ok) return null; return await res.json() } catch(e){return null}
        }, apiBase)
        ...
        if (createdClientId) {
          ...
          const upd = await page.evaluate(async (apiBase, id) => {
            try {
              const payload = { cliente: { name: 'E2E Updated' } }
              const res = await fetch(apiBase + '/api/clients/' + id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
              return res.ok
            } catch (e) { return false }
          }, apiBase, createdClientId)
          console.log('API fallback: update result=', upd)
          // Verify updated
          const verify = await page.evaluate(async (apiBase, id) => { try { const r = await fetch(apiBase + '/api/clients/' + id); if (!r.ok) return null; return await r.json() } catch(e){return null} }, apiBase, createdClientId)
          console.log('API fallback: verify after update=', verify && verify.cliente && verify.cliente.name)
          // Delete
          const del = await page.evaluate(async (apiBase, id) => { try { const r = await fetch(apiBase + '/api/clients/' + id, { method: 'DELETE' }); return r.ok } catch(e){return false} }, apiBase, createdClientId)
