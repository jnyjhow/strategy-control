...
echo "[prod_up] Aguardando backend ficar saudável (http://localhost:3333/api/clients)"
MAX_WAIT=60
SLEEP=2
elapsed=0
until curl -sSf --max-time 2 http://localhost:3333/api/clients >/dev/null 2>&1; do
  if [ $elapsed -ge $MAX_WAIT ]; then
    echo "[prod_up] Timeout esperando backend. Veja os logs: docker compose -f $BACKEND_COMPOSE logs strategy-backend-prod"
    exit 1
  fi
  printf "."
  sleep $SLEEP
