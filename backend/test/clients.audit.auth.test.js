...
const tmpFile = path.join(__dirname, `test-sqlite-audit-auth-${Date.now()}.db`)
...
describe('Clients audit endpoint auth', () => {
  test('audit endpoint protected: 403 without admin, 200 with X-Admin header', async () => {
    // create and delete to generate audit
    const created = await request(app).post('/api/clients').send({ cliente: { name: 'Auth Test', email: 'a@b.com' } })
    const id = created.body.id
    await request(app).delete(`/api/clients/${id}`).set('X-User-Id', 'u')

  // try to fetch audit without admin privileges
  const r1 = await request(app).get(`/api/clients/${id}/audit`)
  ...
  const r2 = await request(app).get(`/api/clients/${id}/audit`).set('Authorization', `Bearer ${adminToken}`)
