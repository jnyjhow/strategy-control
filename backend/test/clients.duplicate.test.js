...
describe('SQLite duplicate cpf_cnpj', () => {
  ...
  beforeAll(() => {
    ...
    try { fs.unlinkSync(sqliteFile) } catch (e) {}
  ...
  test('creating duplicate cpf_cnpj returns 409 structured error', async () => {
    const cliente = { name: 'A', email: 'a@example.com', cpf_cnpj: '123.456.789-09' }
  const res1 = await request(app).post('/api/clients').send({ cliente })
    expect(res1.status).toBe(201)
    // attempt to create duplicate (same digits, different mask)
    const cliente2 = { name: 'B', email: 'b@example.com', cpf_cnpj: '12345678909' }
  const res2 = await request(app).post('/api/clients').send({ cliente: cliente2 })
